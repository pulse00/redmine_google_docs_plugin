...
Modalbox.Methods = {
  ...
  _init: function(options) {
    ...
    this.MBwindow = new Element("div", {
      id: "MB_window",
      style: "display: none; top:"+document.viewport.getScrollOffsets()['top'] + 'px;'
    }).update(
    ...
    this.MBclose = new Element("div").update(new Element("a", {
      id: "MB_close",
  ...
  hide: function(options) { // External hide method to use from
    if(this.initialized) {
      var beforeHide = this.options["beforeHide"];
      if(this.event("beforeHide") != false) { 
        if(options)
          Object.extend(this.options, options); // Passing callbacks
      ...
      { // if the callback failed, we still
        this.options["beforeHide"] = beforeHide; // the call to this.event removes callback from
      }
  ...
  resize: function(byWidth, byHeight, options) { // Change size of MB without loading content
    ...
    //var newLeft = this.options.left != null && this.options.left != undefined ? this.options.left :
    ((oWidth - newWidth)/2);
  ...
  resizeToContent: function(options){
    ...
    if($('scheduledTicket')==null){
      ...
      $(this.MBwindow).setStyle({
        width: ($('scheduledTicket').getWidth() + 24) + "px",
        height: ($('scheduledTicket').getHeight() + this.MBheader.getHeight() + 14) + "px"
      })
    }
  },
...
//        }
  ...
  _setPosition: function () {
    if (this.options.left){
      this.MBwindow.style.left = this.options.left+'px';
    } else {
      this.MBwindow.style.left = Math.round((Element.getWidth(document.body) -
        Element.getWidth(this.MBwindow)) / 2 ) + "px";
    }
    if (this.options.top){
